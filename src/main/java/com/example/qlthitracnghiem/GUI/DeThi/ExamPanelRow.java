    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnDeleteActionPerformed
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                String testCodeOfDeletedExam = examBUS.getTestCodeByExamCode(examCode);
                int result = examBUS.delete(examCode);

                if (result == 1) {
                    JOptionPane.showMessageDialog(parentPanel, "Xóa đề thi thành công!", "Thành công",
                            JOptionPane.INFORMATION_MESSAGE);
                    TestDTO test = testBUS.getTestByTestCode(testCodeOfDeletedExam);
                    if (parentPanel != null && test != null) {
                        parentPanel.showExamPanel(test);
                    }
                } else {
